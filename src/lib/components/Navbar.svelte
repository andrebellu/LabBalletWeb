<script>
    ...
    const closeCorsi = () => {
        hideTimer = setTimeout(() => (corsiOpen = false), 180);
    };
</script>
...
>
    <div
        class="max-w-7xl mx-auto px-4 lg:px-8 flex items-center justify-between
    ...
    >
        ...
        <!-- Desktop -->
        <ul
            class="hidden lg:flex items-center gap-9 text-[11px] uppercase tracking-[0.18em]"
        >
            <li
                class="relative"
                on:mouseenter={openCorsi}
                on:mouseleave={closeCorsi}
            >
                <a href="/#corsi" class="link-underline py-2 inline-block"
                    >Corsi</a
                >
                ...
                {#if corsiOpen}
                    <ul class="absolute left-0 top-full pt-4 min-w-[180px]">
                        <div
                            class="bg-white text-neutral-900 shadow-xl border border-black/5 py-2"
                        >
                            <li>
                                <a
                                    href="/danza"
                                    class="block px-5 py-2.5 hover:bg-neutral-100 transition-colors"
                                    >Danza</a
                                >
                            </li>
                            <li>
            ...
            {#each links.slice(2) as l}
                <li>
                    <a href={l.href} class="link-underline py-2 inline-block"
                        >{l.label}</a
                    >
                </li>
            {/each}
        ...
        >
            <span
                class="block h-[1.5px] w-7 bg-current transition-all duration-300
    ...
    <div class="h-full flex flex-col justify-center px-8 pt-20">
        {#each links as l, i}
            ...
            >
                ...
                <span
                    class="ml-auto text-neutral-300 group-hover:text-neutral-900 transition-colors"
                    >→</span
                >
            </a>
        {/each}
        ...
        <div
            class="mt-10 transition-all duration-500 {open
                ? 'opacity-100'
                : 'opacity-0'}"
